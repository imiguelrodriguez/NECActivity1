...
class NeuralNetCross:
    ...
    **Example**::
    ...
    def __init__(self, layers, epochs=100, learn_rate=0.001, momentum=0.9, fun="tanh", val_percentage=0.2, save=False, k_fold_val=2):
        self.L = len(layers)
        self.n = layers.copy()
        self.save = save
        self.epochs = epochs
    ...
    def fit(self, X_train: np.ndarray, X_val: np.ndarray, y_train: np.ndarray, y_val: np.ndarray, fold: int) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            train_indices = np.random.permutation(num_patterns)
            ...
            print(
                f"Epoch {epoch + 1}/{self.epochs} - Train Error: {train_error:.4f}, Validation Error: {val_error:.4f}")

        # Accumulate losses for averaging across folds
        self.avg_train_loss[fold] = train_loss
        self.avg_val_loss[fold] = val_loss

        print(
            f"Fold {fold + 1} completed. Avg Train Error: { np.mean(self.avg_train_loss[fold]):.4f}, Avg Validation Error: { np.mean(self.avg_val_loss[fold]):.4f}\n")
    ...
    def loss_epochs(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        :rtype: tuple
    """
        return np.array(self.avg_train_loss), np.array(self.avg_val_loss)
    ...
    def plot_loss(self):
        """
        Plot the training and validation loss for each fold after the training process.

        This method visualizes the training and validation loss over epochs for each fold,
        helping to analyze the model's performance during k-fold cross-validation.
        ...
        for fold in range(len(self.avg_train_loss)):
            ...
            ax.set_ylabel('Loss')
            ax.set_title(f'Fold {fold + 1}: Training and Validation Loss \n(learn_rate={self.learn_rate}, '
                         f'mom={self.momentum}, act_func={self.fun}, layers={self.n})')

            # Add legend
            ax.legend()

            # Save the plot for the fold
            plot_file = f"plots/plot_fold{fold + 1}_{self.fun}_{self.epochs}_{self.learn_rate}_{self.momentum}_{self.n}.png"
            fig.savefig(plot_file, format='png', bbox_inches='tight')
            print(f"Plot for fold {fold + 1} saved to {plot_file}.")
